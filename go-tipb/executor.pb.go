...
type IndexScan struct {
	...
	FtsRanges        []KeyRange    `protobuf:"bytes,8,rep,name=fts_ranges,json=ftsRanges" json:"fts_ranges"`
}
...
func (m *IndexScan) GetFtsRanges() []KeyRange {
	if m != nil {
		return m.FtsRanges
	}
	return nil
...
func (m *IndexScan) MarshalToSizedBuffer(dAtA []byte) (int, error) {
	...
	var l int
	_ = l
	if len(m.FtsRanges) > 0 {
		for iNdEx := len(m.FtsRanges) - 1; iNdEx >= 0; iNdEx-- {
			{
				size, err := m.FtsRanges[iNdEx].MarshalToSizedBuffer(dAtA[:i])
				if err != nil {
					return 0, err
				}
				i -= size
...
func (m *IndexScan) Unmarshal(dAtA []byte) error {
	...
	for iNdEx < l {
		...
		case 8:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field FtsRanges", wireType)
			}
			var msglen int
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
			...
			m.FtsRanges = append(m.FtsRanges, KeyRange{})
			if err := m.FtsRanges[len(m.FtsRanges)-1].Unmarshal(dAtA[iNdEx:postIndex]); err != nil {
				return err
			}
			iNdEx = postIndex
		default:
