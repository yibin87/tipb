...
type GroupingFunctionMetadata struct {
	...
	GroupingIds      []uint64 `protobuf:"varint,2,rep,name=grouping_ids,json=groupingIds" json:"grouping_ids,omitempty"`
	XXX_unrecognized []byte   `json:"-"`
}
...
func (m *GroupingFunctionMetadata) GetVersion() uint32 {
...
func (m *GroupingFunctionMetadata) GetGroupingIds() []uint64 {
	if m != nil {
		return m.GroupingIds
	}
	return nil
}
...
func (m *GroupingFunctionMetadata) MarshalTo(dAtA []byte) (int, error) {
	...
	if len(m.GroupingIds) > 0 {
		for _, num := range m.GroupingIds {
			dAtA[i] = 0x10
			i++
	...
	for iNdEx < l {
		...
		case 2:
			if wireType == 0 {
				...
				m.GroupingIds = append(m.GroupingIds, v)
			} else if wireType == 2 {
				var packedLen int
				for shift := uint(0); ; shift += 7 {
					if shift >= 64 {
				...
				return fmt.Errorf("proto: wrong wireType = %d for field GroupingIds", wireType)
			}
		default:
			iNdEx = preIndex
			skippy, err := skipMetadata(dAtA[iNdEx:])
...
var fileDescriptorMetadata = []byte{
	...
	0x30, 0x29, 0x30, 0x6a, 0xf0, 0x04, 0x41, 0x79, 0x4a, 0xe1, 0x5c, 0x12, 0xee, 0x45, 0xf9, 0xa5,
	0x05, 0x99, 0x79, 0xe9, 0x6e, 0xa5, 0x79, 0xc9, 0x25, 0xc8, 0x4e, 0x91, 0xe0, 0x62, 0x2f, 0x4b,
	0x2d, 0x2a, 0x86, 0xb9, 0x84, 0x37, 0x08, 0xc6, 0x15, 0x52, 0xe4, 0xe2, 0x49, 0x87, 0xea, 0x8a,
	0xcf, 0x4c, 0x01, 0x99, 0xc9, 0xac, 0xc1, 0x12, 0xc4, 0x0d, 0x13, 0xf3, 0x4c, 0x29, 0x76, 0xd2,
	0x3c, 0xf1, 0x48, 0x8e, 0xf1, 0xc2, 0x23, 0x39, 0xc6, 0x07, 0x8f, 0xe4, 0x18, 0x67, 0x3c, 0x96,
	0x63, 0xe0, 0x12, 0x4d, 0xce, 0xcf, 0xd5, 0x03, 0x49, 0x27, 0x27, 0x16, 0xe8, 0x95, 0x64, 0xa6,
	0x24, 0xe9, 0x81, 0x42, 0x26, 0x80, 0x11, 0x10, 0x00, 0x00, 0xff, 0xff, 0xf9, 0xd5, 0xe1, 0x84,
	0x32, 0x01, 0x00, 0x00,
}
